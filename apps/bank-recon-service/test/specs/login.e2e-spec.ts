...
describe('Login', () => {
	...
	it('should fail with invalid DTO', async () => {
		...
		const res = await request(`http://localhost:${PORT}`).post('/auth/login').send(invalidPayload);
	...
	it('should succeed when DTO is valid', async () => {
		...
		const res = await request(`http://localhost:${PORT}`).post('/auth/login').send(validPayload).set('Accept', 'application/json');
